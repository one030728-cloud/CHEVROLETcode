...
- **기본 화면(예약)**: 방문자가 차량번호 → 전화번호를 입력하면 대기번호를 발급받습니다.
  접수 시점에 대기중인 손님이 없으면(앞에 아무도 없으면) 즉시 "고객님의 순서입니다" 알림톡이 나가고,
  대기중인 손님이 있으면 "앞으로 N명 남았습니다" 접수 안내가 나갑니다. 이후 매장에서 순서를 호출하면
  (`POST /api/queue/call-next`) 다음 대기 손님에게 "고객님의 순서입니다" 알림톡이 나갑니다.
...
| 메서드/경로 | 설명 | 인증 |
| --- | --- | --- |
| `POST /api/reservations` | 차량번호+전화번호 등록, 대기번호 발급 (알림톡은 아직 안 보냄) | 없음 (10분당 5회 레이트리밋) |
| `POST /api/queue/call-next` | 대기중인 다음 손님을 호출, "순서입니다" 알림톡 발송 | `x-admin-token` |
| `GET /api/reservations/failed` | 순서 호출 알림톡 발송 실패 건 조회 | `x-admin-token` |
