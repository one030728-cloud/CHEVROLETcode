...
- **결제 화면**: 결제금액 입력 → (실제 단말기 결제) → 전화번호를 입력하면 즉시 전자영수증 알림톡이 발송되고
  ...
  **차량번호는 결제 화면에서 다시 입력받지 않습니다** — 입력한 전화번호로 그 손님의 예약 기록을 찾아 차량번호+정비항목을
  자동으로 가져와 영수증에 같이 넣습니다(`findLatestReservationByPhone`, 같은 날 예약을 우선하고 없으면 그 번호로 등록된
  가장 최근 예약을 씀). 예약 없이 바로 결제하러 온 손님은 매칭되는 기록이 없어 차량번호/정비항목 없이 영수증이 나갑니다.
...
- `#{정비항목}` — 예약 시 선택한 정비 종류 (예: 엔진오일 교체). 예약 접수/순서호출/영수증 템플릿에서 사용
  (영수증에서는 전화번호로 찾은 예약 기록에서 가져오며, 매칭되는 예약이 없으면 빈 값)
